...
public class GlobeCoordinate extends Coordinate {
    ...
    public double getDistance(Coordinate coordinate) {
        long rd = Math.abs(coordinate.getXCoord() - this.getXCoord());
        long rdReverse = this.getWorld().getXSize() - rd;
        long cd = Math.abs(coordinate.getYCoord() - this.getYCoord());
        long cdReverse = this.getWorld().getYSize() - cd;

        // returns the minimum of X or Y, because they can go the other way around the map.
        return Math.min(rd, rdReverse) + Math.min(cd, cdReverse);
    }
}
